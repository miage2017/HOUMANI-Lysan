...
public class Serveur_Ameliore {
	...
	public void run() {
		...
		while (true) {
			...
			//Créer un thread serviceClient.start
			Thread thread = new Thread(new ServiceClient(ma_connection));
			thread.start();
		}
	}
	...
	private Boolean Service_Client(Socket la_connection) throws IOException {
		...
		int line_num = 0;
		/*
		 * On lit le flux_entrant ligne à ligne ATTENTION : La fonction readline est
	...
	public static void main(String[] args) {
		...
		if (args.length != 1) {
			System.err.println("usage: java " + Serveur_Ameliore.class.getCanonicalName() + " serverPort");
			System.exit(-1);
		}
		try {
